      ...
      <div className="border rounded-md overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-100">
            <tr><th className="p-2 text-left text-black">ID</th><th className="p-2 text-left text-black">제목</th><th className="p-2 text-left text-black">타입</th><th className="p-2 text-left text-black">상태</th><th className="p-2"></th></tr>
          </thead>
          <tbody>
            {items.map(m => (
              <tr key={m.id} className="border-t">
                <td className="p-2 text-white">{m.id}</td>
            ...
            {items.length === 0 && (
              <tr><td colSpan={5} className="p-4 text-center text-white">메시지가 없습니다.</td></tr>
            )}
          </tbody>
        </table>
      </div>
